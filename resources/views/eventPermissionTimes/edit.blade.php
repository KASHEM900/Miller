...
<div class="card">
    <div class="card-header flex justify-between">
        <span class="text-2xl"> ইভেন্টস পারমিশন সময়নিরুপণ হালনাগাদ</span>
    ...
    <form action="{{ route('eventPermissionTimes.update',$eventPermissionTime->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
         ...
         <div class="row">
             <table>
                 <tbody>
                         <tr>
                             <td >ইভেন্ট<span style="color: red;"> * </span></td>
                             <td><strong>:</strong> </td>
                             <td>
                             <select name="event_id" id="event_id" class="form-control" title="অনুগ্রহ করে ইভেন্ট বাছাই করুন" required="">
                                  <option value="">ইভেন্ট</option>
                                   ...
                                   <option value="{{ $event->event_id}}"{{ $event->event_id == $eventPermissionTime->event_id ? 'selected' : '' }}>{{ $event->event_name}}</option>
                                   @endforeach
                             </select>
                             </td>
                         </tr>
                         <tr>
                             ...
                             <td class="input-append date" name="perm_start_time" id="perm_start_time">
                                ...
                                value="{{$eventPermissionTime->perm_start_time}}" />
                                 <span class="add-on">
                         ...
                         <tr>
                             ...
                             <td><strong>:</strong></td>
                               <td class="input-append date" name="perm_end_time" id="perm_end_time">
                                  <input class="form-control" data-format="yyyy/MM/dd hh:mm:ss" 
                                  name="perm_end_time" type="text" id="perm_start_time" 
                                  value="{{$eventPermissionTime->perm_end_time}}"/>
                                    <span class="add-on">
                                    <i data-time-icon="icon-time" data-date-icon="icon-calendar"></i>
                                  </span>
                            </td>
