        ...
        <div class="col-md-12">
            <div class="card"  id="report_print_summery">
                <div class="card-header text-2xl flex justify-between" style="text-align: center">
                    <h4>চালকলের সংক্ষিপ্ত প্রতিবেদন {{ $headertext }} - কর্পোরেট - (চালের ধরন: {{$chal_type}})</h4>
                    <div>
                    @if($level==1)
                        <a class="btn btn-secondary print_hide" href="{{ route('summarycorporate', $chal_type_id) }}">
                    @elseif($level==2)
                        <a class="btn btn-secondary print_hide" href="{{ route('summarycorporatewithdivision', [$returnid, $chal_type_id]) }}">
                    @elseif($level==3)
                        <a class="btn btn-secondary print_hide" href="{{ route('summarycorporatewithdistrict', [$returnid, $chal_type_id]) }}">
                    @endif
                    ...
                    <table width="100%" border="1" cellspacing="0" cellpadding="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="px-1">
                                        @if($level==0)
                                            <a href="{{ route('summarycorporatewithdivision', [$miller_info->id, $chal_type_id]) }}">
                                        @elseif($level==1)
                                            <a href="{{ route('summarycorporatewithdistrict', [$miller_info->id, $chal_type_id]) }}">
                                        @elseif($level==2)
                                            <a href="{{ route('summarycorporatewithupazilla', [$miller_info->id, $chal_type_id,]) }}">
                                        @elseif($level==3)
