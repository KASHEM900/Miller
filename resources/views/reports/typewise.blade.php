        ...
        <div class="col-md-12">
            <div class="card" id="miller_list">
                <div class="card-header text-2xl">
                    ...
                    <form action="{{ route('exporttypewise') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="division_id"  value="{{$division_id}}">
                        <input type="hidden" name="district_id"  value="{{$district_id}}">
                        <input type="hidden" name="chal_type_id"  value="{{$chal_type_id}}">
                        <input type="hidden" name="mill_type_id"  value="{{$mill_type_id}}">

                        <span class="print_hide" style="float: right;">
                            <input type="button" class="btn btn-secondary" value="প্রিন্ট করুন" onclick="printTypeWiseReport()" />
                            ...
                            <table cellpadding="2px">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>
                                            <select name="chal_type_id" id="chal_type_id" class="form-control" title="অনুগ্রহ করে  চালের ধরন বাছাই করুন">
                                                <option value="">চালের ধরন</option>
                                                @foreach($chalTypes as $chalType)
                                                <option value="{{ $chalType->chal_type_id}}" {{ ( $chalType->chal_type_id == $chal_type_id) ? 'selected' : '' }}>{{ $chalType->chal_type_name}}</option>
                                                @endforeach
                                            </select>
                                        </td>
                ...
                <span style="color:red; padding-left:20px;margin-top:-15px;"> বিভাগ, জেলা,  চালের ধরন ও চালকলের ধরন বাছাই করতে হবে</span>
...
<div class="card-body">
    <table id="table_id" class="table table-bordered" cellspacing="0" width="100%">
        ...
        <thead>
            <tr>
                <th>ক্রমিক নং</th>
                <th width="8%">ফর্ম নম্বর	@sortablelink('form_no', '')</th>
                ...
                <!--<th width="11%">পাক্ষিক ক্ষমতা(ধান) @sortablelink('millar_p_power', '') </th>-->
                <th width="11%">পাক্ষিক ক্ষমতা(চাল)@sortablelink('millar_p_power_chal', '') </th>
                <th width="9%">উপজেলা @sortablelink('upazilla.upazillaname', '')</th>
                <th width="9%">জেলা @sortablelink('district.distname', '')</th>
                <th width="9%">চালকলের অবস্থা</th>
            </tr>
        </thead>
